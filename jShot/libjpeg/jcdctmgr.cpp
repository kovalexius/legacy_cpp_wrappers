  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    case JDCT_ISLOW:
      ...
      if (fdct->divisors[qtblno] == NULL) 
			{
				fdct->divisors[qtblno] = (DCTELEM *) (*cinfo->mem->alloc_small) (( sJpeg_common_struct *) cinfo, JPOOL_IMAGE, DCTSIZE2 * SIZEOF(DCTELEM));
      }
...
#define CONST_BITS 14
	...
	if (fdct->divisors[qtblno] == NULL) 
	{
	  fdct->divisors[qtblno] = (DCTELEM *)
	    (*cinfo->mem->alloc_small) (( sJpeg_common_struct*) cinfo, JPOOL_IMAGE,	DCTSIZE2 * SIZEOF(DCTELEM));
	}
	dtbl = fdct->divisors[qtblno];
	for (i = 0; i < DCTSIZE2; i++) {
	  dtbl[i] = (DCTELEM)
...
void jinit_forward_dct ( sJpeg_compress_struct *cinfo )
{
  ...
  fdct = (my_fdct_ptr)
    (*cinfo->mem->alloc_small) (( sJpeg_common_struct *) cinfo, JPOOL_IMAGE, SIZEOF(my_fdct_controller));
  cinfo->fdct = ( sJpeg_forward_dct *) fdct;
