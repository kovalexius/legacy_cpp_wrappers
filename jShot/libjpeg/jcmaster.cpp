...
/* Set up the scan parameters for the current scan */
{
  ...
  if (cinfo->scan_info != NULL) 
    ...
    const sJpeg_scan_info * scanptr = cinfo->scan_info + master->scan_number;
...
/* cinfo->comps_in_scan and cinfo->cur_comp_info[] are already set */
{
  ...
  if (cinfo->comps_in_scan == 1) 
    ...
    /* Overall image size in MCUs */
    cinfo->MCUs_per_row = (unsigned int)
      jdiv_round_up((long) cinfo->jpeg_width,
		    (long) (cinfo->max_h_samp_factor * cinfo->block_size));
    cinfo->MCU_rows_in_scan = (unsigned int)
...
/* Do computations that are needed before master selection phase */
{
  ...
  /* Width of an input scanline must be representable as unsigned int. */
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    /* Size in DCT blocks */
    compptr->width_in_blocks = (unsigned int)
      jdiv_round_up((long) cinfo->jpeg_width * (long) compptr->h_samp_factor,
		    (long) (cinfo->max_h_samp_factor * cinfo->block_size));
    compptr->height_in_blocks = (unsigned int)
      jdiv_round_up((long) cinfo->jpeg_height * (long) compptr->v_samp_factor,
		    (long) (cinfo->max_v_samp_factor * cinfo->block_size));
    /* Size in samples */
    compptr->downsampled_width = (unsigned int)
      jdiv_round_up((long) cinfo->jpeg_width *
		    (long) (compptr->h_samp_factor * compptr->DCT_h_scaled_size),
		    (long) (cinfo->max_h_samp_factor * cinfo->block_size));
    compptr->downsampled_height = (unsigned int)
      jdiv_round_up((long) cinfo->jpeg_height *
   ...
   */
  cinfo->total_iMCU_rows = (unsigned int)
    jdiv_round_up((long) cinfo->jpeg_height,
		  (long) (cinfo->max_v_samp_factor * cinfo->block_size));
}
...
{
	sJpeg_scan_info * scanptr;
  int idxout, idxin;

  /* Circumvent const declaration for this function */
  scanptr = ( sJpeg_scan_info *) cinfo->scan_info;
  idxout = 0;
...
void jinit_c_master_control ( sJpeg_compress_struct *cinfo, boolean transcode_only)
{
  my_comp_master* master;
	...
	master = (my_comp_master*)(*cinfo->mem->alloc_small) ( (sJpeg_common_struct*) cinfo, JPOOL_IMAGE, (size_t) sizeof (my_comp_master));
  cinfo->master = (struct sJpeg_comp_master *) master;
